...
%%
%% @see with_api/5
-spec with_api(callbacks(), permission(), mix_hex_core:config(), fun((mix_hex_core:config()) -> Result)) ->
...
%% Example:
%% ```
%% mix_hex_cli_auth:with_api(Callbacks, write, Config, fun(C) ->
...
%%
%% @see with_repo/4
-spec with_repo(callbacks(), mix_hex_core:config(), fun((mix_hex_core:config()) -> Result)) ->
    Result | {error, auth_error()}
when
    Result :: term().
...
%% Example:
%% ```
%% mix_hex_cli_auth:with_repo(Callbacks, Config, fun(C) ->
%%     mix_hex_repo:get_tarball(C, <<"ecto">>, <<"3.0.0">>)
%% end).
%% '''
-spec with_repo(
    callbacks(), mix_hex_core:config(), fun((mix_hex_core:config()) -> Result), opts()
) ->
    Result | {error, auth_error()}
when
    Result :: term().
...
-spec device_auth(callbacks(), mix_hex_core:config(), binary(), opts()) ->
...
%% @private
-spec resolve_api_auth(callbacks(), permission(), mix_hex_core:config()) ->
...
-spec resolve_repo_auth(callbacks(), mix_hex_core:config()) ->
