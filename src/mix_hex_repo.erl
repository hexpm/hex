...
get_package(Config, Name) when is_binary(Name) and is_map(Config) ->
    ...
    Decoder = fun(Data) ->
        case Verify of
            true -> mix_hex_registry:decode_package(Data, repo_name(Config), Name);
            false -> mix_hex_registry:decode_package(Data, no_verify, no_verify)
        end
    end,
...
%%
%% ```
%% > Config = (mix_hex_core:default_config())#{repo_organization => <<"myorg">>},
%% > mix_hex_repo:get_policy(Config, <<"strict-prod">>).
%% {ok, {200, ...,
%%       #{repository => <<"myorg">>,
%%         name => <<"strict-prod">>,
...
get_policy(Config, Name) when is_binary(Name) and is_map(Config) ->
    ...
    Decoder = fun(Data) ->
        case Verify of
            true -> mix_hex_registry:decode_policy(Data, repo_name(Config), Name);
            false -> mix_hex_registry:decode_policy(Data, no_verify, no_verify)
        end
    end,
...
decode(Signed, PublicKey, Decoder, Config) ->
    ...
    case Verify of
        true ->
            case mix_hex_registry:decode_and_verify_signed(Signed, PublicKey) of
                {ok, Payload} ->
                    Decoder(Payload);
                Other ->
                    Other
        ...
        false ->
            #{payload := Payload} = mix_hex_registry:decode_signed(Signed),
            Decoder(Payload)
    end.
