%% Vendored from hex_core v0.16.0 (c4c38cc), do not edit manually

%% @doc
%% Hex HTTP API - Releases.
-module(mix_hex_api_release).
-export([
    delete/3,
    get/3,
    publish/2,
...
%%
%% ```
%% > mix_hex_api_release:get(mix_hex_core:default_config(), <<"package">>, <<"1.0.0">>).
%% {ok, {200, ..., #{
...
get(Config, Name, Version) when is_map(Config) and is_binary(Name) and is_binary(Version) ->
    Path = mix_hex_api:build_repository_path(Config, ["packages", Name, "releases", Version]),
...
%%
%% ```
%% > mix_hex_api_release:publish(mix_hex_core:default_config(), Tarball).
%% {ok, {200, ..., #{
%%      <<"checksum">> => <<"540d210d81f56f17f64309a4896430e727972499b37bd59342dc08d61dff74d8">>,
%%      <<"docs_html_url">> => <<"https://hexdocs.pm/package/1.0.0/">>,
%%      <<"downloads">> => 740,<<"has_docs">> => true,
...
%% '''
%% @end
-spec publish(mix_hex_core:config(), binary()) -> mix_hex_api:response().
...
%%
%% ```
%% > mix_hex_api_release:publish(mix_hex_core:default_config(), Tarball, [{replace, true}]).
%% {ok, {201, ..., #{
%%      <<"checksum">> => <<"540d210d81f56f17f64309a4896430e727972499b37bd59342dc08d61dff74d8">>,
%%      <<"docs_html_url">> => <<"https://hexdocs.pm/package/1.0.0/">>,
%%      <<"downloads">> => 740,<<"has_docs">> => true,
...
%%
%% ```
%% > mix_hex_api_release:delete(mix_hex_core:default_config(), <<"package">>, <<"1.0.0">>).
%% {ok, {204, ..., nil}}
%% '''
%% @end
...
%%
%% ```
%% > mix_hex_api_release:retire(mix_hex_core:default_config(), <<"package">>, <<"1.0.0">>, Params).
%% {ok, {204, ..., nil}}
%% '''
%% @end
-spec retire(mix_hex_core:config(), binary(), binary(), retirement_params()) -> mix_hex_api:response().
retire(Config, Name, Version, Params) when
    is_map(Config) and is_binary(Name) and is_binary(Version)
->
    Path = mix_hex_api:build_repository_path(Config, ["packages", Name, "releases", Version, "retire"]),
...
%%
%% ```
%% > mix_hex_api_release:unretire(mix_hex_core:default_config(), <<"package">>, <<"1.0.0">>).
%% {ok, {204, ..., nil}}
%% '''
%% @end
-spec unretire(mix_hex_core:config(), binary(), binary()) -> mix_hex_api:response().
