defmodule Mix.Tasks.Hex.Build do
  ...
  defp create_release(meta, progress?) do
    tarball = Hex.Tar.create(meta, meta[:files], false)
  ...
  defp dependencies(meta) do
    ...
    Enum.each(include, fn {app, _req, opts} ->
      if opts[:override] do
        Mix.raise "Can't build with overridden dependency #{app}, remove `override: true`"
      end
    end)
