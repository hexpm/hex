...
defmodule Hex.Mint.HTTP1 do
  ...
  a given server. The connection is represented as an opaque struct `%Hex.Mint.HTTP1{}`.
  ...
  module, with the exception that `Hex.Mint.HTTP1` specifically deals with HTTP/1 and HTTP/1.1 while
  ...
  alias Hex.Mint.HTTP1.{Parse, Request, Response}
  alias Hex.Mint.{HTTPError, TransportError, Types}
  ...
  @doc """
  Same as `Hex.Mint.HTTP.connect/4`, but forces an HTTP/1 or HTTP/1.1 connection.
  ...
  # Made public since the %Hex.Mint.HTTP1{} struct is opaque.
  @doc false
  @impl true
  @spec put_proxy_headers(t(), Hex.Mint.Types.headers()) :: t()
