...
defmodule Hex.Mint.HTTP do
  ...
  Single interface for `Hex.Mint.HTTP1` and `Hex.Mint.HTTP2` with support for version
  ...
  does not take care of the response to that request, instead we use `Hex.Mint.HTTP.stream/2`
  ...
  `Hex.Mint.HTTP` makes it easy to identify TCP/SSL messages that are coming from the
  ...
  to a server.
      ...
      {:ok, conn} = Hex.Mint.HTTP.connect(:http, "httpbin.org", 80)

  `conn` is a data structure that represents the connection.

  To send a request, we use `request/5`.

      {:ok, conn, request_ref} = Hex.Mint.HTTP.request(conn, "GET", "/", [], nil)
  ...
  use a simple `receive`. `Hex.Mint.HTTP` provides a way to tell if a message comes
  ...
  `:unknown`. Otherwise, it returns an updated connection and one or more responses.
      ...
      receive do
        message ->
          case Hex.Mint.HTTP.stream(conn, message) do
  ...
  See `t:Hex.Mint.Types.response/0` for the full list of responses returned by `Hex.Mint.HTTP.stream/2`.
  ...
  When using SSL, you can pass in your own CA certificate store or use one provided by Hex.Mint. Mint
  ...
  If you are using OTP 25+ it is recommended to set this option.
      ...
      Hex.Mint.HTTP.connect(:https, host, port, transport_opts: [cacerts: :public_key.cacerts_get()])
  ...
  alias Hex.Mint.{Types, TunnelProxy, UnsafeProxy}
  alias Hex.Mint.Core.{Transport, Util}

  @behaviour Hex.Mint.Core.Conn

  @opaque t() :: Hex.Mint.HTTP1.t() | Hex.Mint.HTTP2.t()
  ...
  defguardp is_proxy_conn(conn) when is_struct(conn, Hex.Mint.UnsafeProxy)
  ...
  `Hex.Mint.HTTP.stream/2` and check for the `:unknown_message` return value.
  ...
  **Note**: this macro is only available if you compile Mint with Elixir 1.10.0 or greater (and
  ...
  ## Examples
      ...
      require Hex.Mint.HTTP

      {:ok, conn, request_ref} = Hex.Mint.HTTP.request(conn, "POST", "/", headers, "")

      receive do
        message when Hex.Mint.HTTP.is_connection_message(conn, message) ->
          Hex.Mint.HTTP.stream(conn, message)
  ...
  ## Options
    ...
    * `:proxy_headers` - a list of headers (`t:Hex.Mint.Types.headers/0`) to pass when using
  ...
  Options for `:https` only:
    ...
    * `:alpn_advertised_protocols` - managed by Hex.Mint. Cannot be overridden.
  ...
  Using a proxy:
      ...
      {:ok, conn} = Hex.Mint.HTTP.connect(:https, "httpbin.org", 443, proxy: proxy)

  Forcing the connection to be an HTTP/2 connection:

      {:ok, conn} = Hex.Mint.HTTP.connect(:https, "httpbin.org", 443, protocols: [:http2])
  ...
  Enable all default cipher suites of Erlang/OTP (release 20.3 or later):
      ...
      {:ok, conn} = Hex.Mint.HTTP.connect(:https, "httpbin.org", 443, opts)
  ...
  def connect(scheme, address, port, opts \\ []) do
    case Keyword.fetch(opts, :proxy) do
      ...
      :error ->
        Hex.Mint.Negotiate.connect(scheme, address, port, opts)
    end
  end
  ...
  def upgrade(old_transport, transport_state, scheme, hostname, port, opts),
    do: Hex.Mint.Negotiate.upgrade(old_transport, transport_state, scheme, hostname, port, opts)
  ...
  ## Examples
      ...
      iex> Hex.Mint.HTTP.protocol(%Hex.Mint.HTTP1{})
      :http1

      iex> Hex.Mint.HTTP.protocol(%Hex.Mint.HTTP2{})
      :http2
  ...
  def protocol(%Hex.Mint.HTTP1{}), do: :http1
  def protocol(%Hex.Mint.HTTP2{}), do: :http2
  def protocol(%Hex.Mint.UnsafeProxy{state: internal_conn}), do: protocol(internal_conn)
  ...
  def initiate(transport, transport_state, hostname, port, opts),
    do: Hex.Mint.Negotiate.initiate(transport, transport_state, hostname, port, opts)
  ...
  ## Examples
      ...
      {:ok, conn} = Hex.Mint.HTTP.close(conn)
  ...
  server but you can still receive data from the server. In this case, `Hex.Mint.HTTP.open?(conn, :read)`
  would return `true` but `Hex.Mint.HTTP.open?(conn, :write)` would return `false`.
  See the "Closed connection" section in the module documentation of `Hex.Mint.HTTP2`.

  If a connection is *completely closed* (that is, `Hex.Mint.HTTP.open?(conn, :read)` returns `false`),
  ...
  > `Hex.Mint.HTTP.open?(conn)` will return `false` if `conn` was closed for writing
  > but is still open for reading. If you need to make sure the connection is
  > completely closed, check that `Hex.Mint.HTTP.open?(conn, :read)` returns `false`.

  ## Examples

      {:ok, conn} = Hex.Mint.HTTP.connect(:http, "httpbin.org", 80)
      Hex.Mint.HTTP.open?(conn)
      #=> true
  ...
  See `Hex.Mint.HTTP1.request/5` for more information.

  For a quick discussion on HTTP/2 streams and requests, see the `Hex.Mint.HTTP2` module and
  `Hex.Mint.HTTP2.request/5`.
  ...
  transfer-encoding when a content-length is not provided (see `Hex.Mint.HTTP1.request/5`).

  ## Examples

      Hex.Mint.HTTP.request(conn, "GET", "/", _headers = [], _body = nil)
      Hex.Mint.HTTP.request(conn, "POST", "/path", [{"content-type", "application/json"}], "{}")
  ...
  `Hex.Mint.HTTP1.stream_request_body/3`.
  ...
  The behaviour is slightly different for HTTP/1 and HTTP/2:
    ...
    * In HTTP/1, trailer headers are only supported if the transfer encoding is set to
      `chunked`. See `Hex.Mint.HTTP1.stream_request_body/3` for more information on chunked
      transfer encoding.
  ...
  use the `trailers` value in the `te` header. For example:
      ...
      Hex.Mint.HTTP.request(conn, "GET", "/", [{"te", "trailers"}], "some body")
  ...
  brace at a time.
      ...
      {:ok, conn, request_ref} = Hex.Mint.HTTP.request(conn, "POST", "/", headers, :stream)
      {:ok, conn} = Hex.Mint.HTTP.stream_request_body(conn, request_ref, "{")
      {:ok, conn} = Hex.Mint.HTTP.stream_request_body(conn, request_ref, "}")
      {:ok, conn} = Hex.Mint.HTTP.stream_request_body(conn, request_ref, :eof)
  ...
  Here's an example of sending trailer headers:
      ...
      {:ok, conn} = Hex.Mint.HTTP.stream_request_body(conn, request_ref, {:eof, trailer_headers})
  ...
  element is a unique reference `t:Hex.Mint.Types.request_ref/0` that identifies the request
  ...
  These are the possible responses that can be returned.
    ...
    * `{:status_reason, request_ref, reason_phrase}` - returned when the server replied
      ...
      `:status_reason` in the list. See `Hex.Mint.HTTP1.connect/4` for more information.
    ...
    * `{:pong, request_ref}` - returned when a server replies to a ping
      ...
      and will never be returned by an HTTP/1 connection. See `Hex.Mint.HTTP2.ping/2`
    ...
    * `{:push_promise, request_ref, promised_request_ref, headers}` - returned when
      ...
      and will never be returned by an HTTP/1 connection. See `Hex.Mint.HTTP2` for more
  ...
  as an argument, and then returns the result of `stream/2`:
      ...
      defp receive_next_and_stream(conn) do
        receive do
          message -> Hex.Mint.HTTP.stream(conn, message)
        end
      end

  Now, we can see an example of a workflow involving `stream/2`.

      {:ok, conn, request_ref} = Hex.Mint.HTTP.request(conn, "GET", "/", _headers = [])
  ...
  See `Hex.Mint.HTTP2.open_request_count/1` for more information.

  ## Examples

      {:ok, conn, _ref} = Hex.Mint.HTTP.request(conn, "GET", "/", [])
      Hex.Mint.HTTP.open_request_count(conn)
      #=> 1
  ...
  ## Examples
      ...
      {:ok, conn, responses} = Hex.Mint.HTTP.recv(conn, 0, 5000)
  ...
  ## Examples
      ...
      {:ok, conn} = Hex.Mint.HTTP.set_mode(conn, :passive)
  ...
  ## Examples
      ...
      {:ok, conn} = Hex.Mint.HTTP.controlling_process(conn, new_pid)
  ...
  Let's see an example of putting a value and then getting it:
      ...
      conn = Hex.Mint.HTTP.put_private(conn, :client_name, "Mint")
      Hex.Mint.HTTP.get_private(conn, :client_name)
      #=> "Mint"
  ...
  ## Examples
      ...
      Hex.Mint.HTTP.get_private(conn, :non_existent)
      #=> nil
  ...
  ## Examples
      ...
      conn = Hex.Mint.HTTP.delete_private(conn, :client_name)
      Hex.Mint.HTTP.get_private(conn, :client_name)
      #=> nil
  ...
  defp conn_apply(%Hex.Mint.HTTP1{}, fun, args), do: apply(Hex.Mint.HTTP1, fun, args)
  defp conn_apply(%Hex.Mint.HTTP2{}, fun, args), do: apply(Hex.Mint.HTTP2, fun, args)
end
