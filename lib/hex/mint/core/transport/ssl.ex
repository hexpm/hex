...
defmodule Hex.Mint.Core.Transport.SSL do
  ...
  @behaviour Hex.Mint.Core.Transport
  ...
  def upgrade(socket, :http, hostname, _port, opts) do
    ...
    Hex.Mint.Core.Transport.TCP.setopts(socket, active: false)
  ...
  def wrap_error(reason) do
    %Hex.Mint.TransportError{reason: reason}
  end
  ...
  def verify_fun(cert, :valid_peer, state) do
    if :hex_mint_shims.pkix_verify_hostname(cert, state, match_fun: &match_fun/2) do
      {:valid, state}
    else
