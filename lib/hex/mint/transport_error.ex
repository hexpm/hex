...
defmodule Hex.Mint.TransportError do
  ...
  A `Hex.Mint.TransportError` struct is an exception, so it can be raised as any
  other exception.
  ...
  for using in logs), you can use `Exception.message/1`:
      ...
      iex> {:error, %Hex.Mint.TransportError{} = error} = Hex.Mint.HTTP.connect(:http, "nonexistent", 80)
  ...
  @typedoc """
  Type for the `Hex.Mint.TransportError` exception struct.
  """
