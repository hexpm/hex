...
defmodule Hex.Mint.HPAX do
  ...
  alias Hex.Mint.HPAX.{Table, Types}
  ...
  This function accepts the following `options`:
    ...
    * `:huffman_encoding` - (since 0.2.0) `:always` or `:never`. If `:always`,
      then Hex.Mint.HPAX will always encode headers using Huffman encoding. If `:never`,
      Hex.Mint.HPAX will not use any Huffman encoding. Defaults to `:never`.
  ...
  ## Examples
      ...
      decoding_context = Hex.Mint.HPAX.new(1000)
      hbf = get_hbf_from_somewhere()
      Hex.Mint.HPAX.decode(hbf, decoding_context)
  ...
  ## Examples
      ...
      encoding_context = Hex.Mint.HPAX.new(1000)
      Hex.Mint.HPAX.encode(headers, encoding_context)
  ...
  and the same `action` is applied to all headers.
    ...
    ## Examples
      ...
      encoding_context = Hex.Mint.HPAX.new(1000)
      Hex.Mint.HPAX.encode(:store, headers, encoding_context)
      #=> {iodata, updated_encoding_context}
