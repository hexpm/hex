defmodule Mix.Tasks.Hex.OwnerTest do
  ...
  test "add owner" do
    ...
    assert {:ok, {200, %{"owned_packages" => %{"owner_package1" => _}}, _}} = Hex.API.User.get("owner_user2")
  end
  ...
  test "list all packages owned by the current user" do
    ...
    owner_package4_msg = "#{package1} - #{Hexpm.url()}/packages/#{package1}"
    owner_package5_msg = "#{package2} - #{Hexpm.url()}/packages/#{package2}"
