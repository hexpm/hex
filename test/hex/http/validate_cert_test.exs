defmodule Hex.HTTP.ValidateCertTest do
  ...
  def partial_chain_fun do
    &Hex.HTTP.SSL.partial_chain([der_encoded_ca_cert()], &1)
  end
  ...
  if Hex.HTTP.SSL.secure_ssl? do
    ...
    test "succeeds to validate with partial chain that is correct" do
      partial_chain_fun = &Hex.HTTP.SSL.partial_chain(Hex.HTTP.Certs.cacerts, &1)
